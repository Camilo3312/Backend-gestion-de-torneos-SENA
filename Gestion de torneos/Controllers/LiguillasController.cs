...
namespace Gestion_de_torneos.Controllers
{
    ...
    public class LiguillasController : ControllerBase
    {
        ...
        {
            string script = @"
                            select l.id, count(e.id) as equipos, (select e.nombreequipo from equipos e where e.partidosganados * 3 = (select max(e.partidosganados * 3)  from equipos e where e.liguilla = l.id ) order by e.partidosperdidos asc, e.partidosganados desc limit 1 ) as equipotop 
